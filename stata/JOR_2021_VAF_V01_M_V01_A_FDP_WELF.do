...
Framework) 2021 and 2023 rounds to the 50by35 schema
...
replicability package (harmonized.do, Jordan 2021/2023 block). The VAF
covers Syrian refugees only (no Jordanian host sample) — both survey
rounds (2021, 2023) are produced by this single script since they share
one raw file, mirroring the source replicability package.
...
50by35 monitoring population (Syrian refugee response only).
...
* identifiers: rebuild hhid from year/Residence/Form/id (no single hhid var).
* year is part of the key because the VAF re-surveys some of the same
* households across rounds, so hhid alone repeats across 2021 and 2023.
tostring id, gen(id_str)
gen hhid = string(year) + "_" + string(Residence) + "_" + Form + "_" + id_str
drop id_str
...
* ---- schema variables ----------------------------------------------------
gen str3 code = "JOR"
* year already present from both files (2021, 2023)
gen      survname    = "VAF"
...
di as result "Saved `outdir'/JOR_2021_VAF_V01_M_V01_A_FDP_WELF.dta (`=_N' refugees, 2021 and 2023 rounds)"
