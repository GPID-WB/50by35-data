/* =====================================================================
04_review_harmonized_batch.do
50by35 pipeline — Step 3 reviewer (batch): approve or reject multiple
harmonized-data transactions via PRIMUS (Stata client).
...
monitoring. A decision is final and cannot be changed.
...
local manifest  "batch/04_harmonized_reviews.csv"

* ---- auto-generate manifest from pending transactions ------------------------
di as result "=== Querying pending harmonized-data transactions ==="
primus query, process(`processid') overallstatus(PENDING)

if _N == 0 {
    di as result "No pending harmonized-data transactions — nothing to review."
    exit
}
...
gen comments = "harmonized data approved for publication"
...
forvalues i = 1/`nrev' {
    ...
    capture noisily {
        ...
        primus download, processid(`processid') tranxid(`tid') indicator

        primus download, xml processid(`processid') tranxid(`tid') ///
            outfile("indicator_`tid'.xml")
...
* ---- summary -----------------------------------------------------------------
di as result _n "=== Batch summary (04_review_harmonized) ==="
forvalues j = 1/`nok' {
